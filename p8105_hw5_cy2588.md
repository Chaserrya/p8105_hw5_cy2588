...
μ when the true value of μ is greater than 3.  
...
is false increased, and the sample average of μ^ will close to the true
value. But when the power of test is relatively lower, some tests for
which the null is false will fail to reject and cause the sample average
of μ^ deviate from the true values.  
